...
function systemPrompt(mindmapId: string | null | undefined): string {
  return [
    ...
    "- To find something by what it says rather than by which map it is in, call search_topics. It looks inside notes, which no outline contains, so reading maps one by one to hunt for a word is both slower and blind to most of what the user wrote.",
    "- Prefer one tool call that does the whole job (add_topics takes a nested tree; rename_topics, move_topics and delete_topics take lists) over many small calls.",
